...
const onSubmit = handleSubmit((values) => {
    ...
    if (!dataToSend.foto_bangunan) {
        delete dataToSend.foto_bangunan;
    }

    router.post(route('admin.buildingUpdate', props.building.id), {
        ...dataToSend,
        _method: 'put',
    }, {
...
<template>
    <div class="flex flex-col gap-8 md:flex-row">
        <form id="add-schedule-form" class="w-full md:w-1/2 space-y-6" @submit="onSubmit">
            ...
            <FormField v-slot="{ componentField }" name="google_maps" :validate-on-blur="!isFieldDirty">
                <FormItem>
                    ...
                    <FormControl>
                        <Textarea placeholder="Masukan iframe google maps" class="resize-none"
                            v-bind="componentField" />
        ...
        <div class="w-full md:w-1/2 space-y-6">
            <GalleryPhoto :buildingID="building.id" :initial-building-photos="buildingPhotos"/>
        </div>
    </div>
</template>
